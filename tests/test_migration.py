...
def test_migration_v1_upgrade():
    ...
    c.execute("CREATE TABLE bgm_library (id INTEGER PRIMARY KEY AUTOINCREMENT, track_name TEXT UNIQUE, valence REAL, energy REAL, tempo INTEGER, tags TEXT)")
    c.execute("INSERT INTO bgm_library (track_name, valence) VALUES ('old_song', 0.8)")
    c.execute("INSERT INTO schema_migrations (version, applied_at) VALUES (1, '2026-01-01')")
    conn.commit()
    conn.close()
