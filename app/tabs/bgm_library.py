...
def _edit_and_confirm_dialog(analysis_result: dict, db_path: str):
    ...
    st.caption(
        f"艺术家: {analysis_result['artist']} | AI 置信度: {analysis_result['confidence']:.2f}"
    )
...
def render_bgm_library_tab(ctx: dict) -> None:
    ...
    with c_input:
        bgm_query = st.text_input(
            "输入歌曲名称 (建议带上艺术家或出处)",
            placeholder="例如：Hans Zimmer - Time (盗梦空间)",
    ...
    if not bgm_list:
        ...
        # 格式化展示
        display_df["验证"] = display_df["验证"].apply(
