...
def get_tags_by_type(db_path: str, tag_type: str) -> list[dict]:
    ...
    c = conn.cursor()
    try:
        c.execute("SELECT id, name, type FROM tags WHERE type = ? ORDER BY name ASC", (tag_type,))
...
def link_tag_to_movie(db_path: str, movie_name: str, tag_id: int, confidence: float = 1.0, source: str = 'manual'):
    conn = get_db_connection(db_path)
    c = conn.cursor()
    try:
        c.execute("""
...
def link_tag_to_quote(db_path: str, quote_id: int, tag_id: int, confidence: float = 1.0, source: str = 'llm'):
    conn = get_db_connection(db_path)
    c = conn.cursor()
    try:
        c.execute("""
...
def link_tag_to_subtitle(db_path: str, subtitle_id: int, tag_id: int, confidence: float = 1.0, source: str = 'manual'):
    conn = get_db_connection(db_path)
    c = conn.cursor()
    try:
        c.execute("""
